...
export const UseCases = () => {
  ...
  // Slowly auto-scroll the cards, looping back to the start and pausing
  // whenever the user hovers/touches or manually scrolls the container.
  useEffect(() => {
    const container = scrollContainerRef.current;
    if (!container || isPaused) return;

    let frameId: number;
    const speed = 0.5; // pixels per frame — slow, steady scroll
  ...
  return (
    <section ref={ref} className="py-12 md:py-16 lg:py-24 bg-section-bg overflow-hidden" id="use-cases" aria-label="กรณีการใช้งาน AI Calling Agent ในธุรกิจต่างๆ">
      <div className="section-container mb-8 md:mb-12 px-4 sm:px-6 md:px-8 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
        <motion.h2
        ...
        <div className="flex gap-3 self-end sm:self-auto shrink-0">
          <Button
            variant="outline"
            size="icon"
            className="rounded-full w-11 h-11 md:w-12 md:h-12 bg-background border-border/50 hover:bg-muted transition-colors"
            onClick={() => scroll('left')}
            aria-label="Previous"
          >
